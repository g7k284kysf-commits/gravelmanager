...
class PerformanceMetricsRepository:
    ...
    def training_bounds(self, tenant_id: int, user_id: int) -> tuple[date | None, date | None]:
    ...
    def daily_loads(
        self, tenant_id: int, user_id: int, start_date: date, end_date: date
    ) -> dict[date, DailyLoad]:
    ...
    def metrics_between(
        ...
        return list(
            self.session.scalars(
                ...
                .where(
                    DailyPerformanceMetric.tenant_id == tenant_id,
    ...
    def latest(self, tenant_id: int, user_id: int) -> DailyPerformanceMetric | None:
    ...
    def at_date(
        self, tenant_id: int, user_id: int, metric_date: date
    ) -> DailyPerformanceMetric | None:
        return self.session.scalar(
    ...
    def existing_from(
        self, tenant_id: int, user_id: int, start_date: date
    ) -> dict[date, DailyPerformanceMetric]:
        return {
    ...
    def delete_all(self, tenant_id: int, user_id: int) -> None:
    ...
    def delete_before(self, tenant_id: int, user_id: int, first_date: date) -> None:
        self.session.execute(
            delete(DailyPerformanceMetric).where(
                DailyPerformanceMetric.tenant_id == tenant_id,
                DailyPerformanceMetric.user_id == user_id,
