...
export function ConnectionDetail({ connectionId }: { connectionId: number }) {
  ...
  const load = useCallback(async () => {
    try {
      const [nextConnection, allSyncs, allEvents] = await Promise.all([
        getConnection(connectionId), getSyncs(), getIntegrationEvents(),
      ]);
      setConnection(nextConnection);
      setSyncs(allSyncs.filter((item) => item.connection_id === connectionId));
      setEvents(allEvents.filter((item) => item.connection_id === connectionId));
    } catch {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] bg-white p-6 shadow-card sm:p-8">
        ...
        <div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => void test()} className="rounded-full bg-ink px-5 py-2 text-sm font-bold text-white">Test connection</button><button type="button" onClick={() => void sync()} className="rounded-full border border-moss/20 px-5 py-2 text-sm font-bold">Start sync</button><button type="button" disabled={connection.status === "revoked"} onClick={() => void revoke()} className="rounded-full px-5 py-2 text-sm font-bold text-red-700 disabled:opacity-40">Revoke</button></div>
